...
import { assertIsArray, assertIsBuffer, assertIsHexString } from "./helpers.ts";
...
interface BufferArray {
  [index: number]: (BufferArray | Buffer);
}
...
export function baToJSON(ba: BufferArray): string[];
export function baToJSON(ba: Buffer | BufferArray): string | string[] {
  if (Array.isArray(ba)) {
    const array: string[] = [];
