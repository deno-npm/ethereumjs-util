...
import { assert, assertEquals, assertThrows } from "./deps.js";
...
Deno.test("Account serializes correctly", function () {
  ...
  const accountRlp = rlp.encode(
    [raw.nonce, raw.balance, raw.stateRoot, raw.codeHash],
  );
...
Deno.test("toChecksumAddress EIP1191", function () {
  for (
    const [chainId, addresses] of Object.entries(eip1191ChecksummAddresses)
  ) {
...
Deno.test("isValidChecksumAddress EIP1191", function () {
  ...
  assertEquals(
    isValidChecksumAddress("0x2f015c60e0be116b1f0cd534704db9c92118fb6a", 1),
    false,
  );
...
Deno.test("isValidAddress", function () {
  assertEquals(
    isValidAddress("0x2f015c60e0be116b1f0cd534704db9c92118fb6a"),
    true,
  );
  assertEquals(
  ...
  assertEquals(
    isValidAddress("0x2f015c60e0be116b1f0cd534704db9c92118fb6"),
    false,
  );
  assertEquals(
